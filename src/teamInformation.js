import DSCLeadPhoto from './assets/images/sunanda_somwase.jpeg'
import Gaurav from './assets/images/gaurav_khairnar.jpg'

export const DSCLeadInformation = {
    name : 'Sunanda Somwase',
    photo : './assets/images/sunanda_somwase.jpg',
    position: 'DSC Lead',
    quote : 'There are risks and costs to action. But they are far less than the long range risks of comfortable inaction.'
}

export const teamInformation  = {

    core: [
        {
            name : 'Sunanda Somwase',
            photo : DSCLeadPhoto,
            position: 'DSC Lead',
        },
        {
            name : 'Adwait Bhope',
            photo : 'https://drive.google.com/open?id=1KUD-U1x-A_mJ9S1v_QWw648oXOyWEaSq',
            position: 'Technical Head', 
        },
        {
            name : 'Gaurav Khairnar',
            photo : Gaurav,
            position: 'Associate Technical Head', 
        },
        {
            name : 'Shivam Rajput',
            photo : 'https://drive.google.com/open?id=1pu48w9go7a9rI10drLG_S2r07GK0l5lJ',
            position: 'Management Head', 
        },
        {
            name : 'Yash Mehta',
            photo : 'https://drive.google.com/open?id=1LlmivdCu2CN_VRt20WtRnwbjXb1uYkDi',
            position: 'Operations Head', 
        },
        {
            name : 'Kshitij Chitnis',
            photo : 'https://drive.google.com/open?id=19WZVAb4vpxlpBxJiSE1ujT75gRDRt-Wr',
            position: 'Design and Production Head', 
        },
        {
            name : 'Vinod Kamat',
            photo : 'https://drive.google.com/open?id=1p9_UEXrEhh1Z0g2RJQ7ahmHAYpfuQTaU',
            position: 'Project Management Head', 
        },
        {
            name : 'Atharva Dhekne',
            photo : 'https://drive.google.com/open?id=17lOr1xK-fQmqANAZawGFZbFV_oQ9FQBU',
            position: 'Chief Editor', 
        },

    ],
    team : [
        {
            name : 'Amey Satwe',
            position: 'Cloud Expert',
        },
        {
            name : 'Yatish Kelkar',
            position: 'UI UX Expert',
        },
        {
            name : 'Mandar Bhide',
            position: 'Flutter Expert',
        },
        {
            name : 'Shruti Yadav',
            position: 'Flutter Expert',
        },
        {
            name : 'Rohit Dalvi',
            position: 'Web Expert',
        },
        {
            name : 'Tejas Shah',
            position: 'Web Expert',
        },
        {
            name : 'Shreya Kajbaje',
            position: 'Android Expert',
        },
        {
            name : 'Prashant Bhandari',
            position: 'Andoid Expert',
        },
        {
            name : 'Pranav Jojan',
            position: 'Machine Learning Expert',
        },
        {
            name : 'Rutuja Bhure',
            position: 'Equity Head',
        },
        {
            name : 'Ajinkya Sahu',
            position: 'Events Organiser',
        },
        {
            name : 'Adwait Madhekar',
            position: 'Events Organiser',
        },
        {
            name : 'Prasannaraj Patil',
            position: 'Logistics Officer',
        },
        {
            name : 'Rasika Deshpande',
            position: 'Outreach Officer',
        },
        {
            name : 'Shweta Sonawane',
            position: 'Design Specialist',
        },
        {
            name : 'Vaibhav Karav',
            position: 'Social Media Specialist',
        },
        {
            name : 'Akshay Ingle',
            position: 'Marketing Specialist',
        },
        {
            name : 'Pranav Kulkarni',
            position: 'Project Manager',
        },
        {
            name : 'Harshada Gundecha',
            position: 'Project Manager',
        },
        {
            name : 'Mahesh Nahak',
            position: 'Content Writer',
        },
        {
            name : 'Shreyas Garud',
            position: 'Content Writer',
        },
        {
            name : '',
            position: '',
        },
        {
            name : '',
            position: '',
        },
        {
            name : '',
            position: '',
        },
    ]
}
